...
def get_sht31_temp(i2c_sensor, temp_unit='f'):
    ...
    temp_conv = convert_tempf(temp) if temp_unit == 'f' else temp
    return round(temp_conv, 2)
...
def get_sensor_state(prev_w1temp=0):
    ...
    gc_temp = get_sht31_temp(gc_temp_humd_i2c)
    gc_humd = get_sht31_humd(gc_temp_humd_i2c)
    ext_temp = get_sht31_temp(ext_temp_humd_i2c)
